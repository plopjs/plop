...
describe('prompt-bypass-confirm', function () {
	...
	test('verify bad bypass input', async function () {
		await expect(() => promptBypass([prompts[0]], ['asdf'], {is: plop})).rejects.toThrow();
		await expect(() => promptBypass([prompts[0]], ['1'], {is: plop})).rejects.toThrow();
		await expect(() => promptBypass([prompts[0]], ['0'], {is: plop})).rejects.toThrow();
		await expect(() => promptBypass([prompts[0]], ['no way'], {is: plop})).rejects.toThrow();
		await expect(() => promptBypass([prompts[0]], ['NOOOOOO'], {is: plop})).rejects.toThrow();
	});
});
