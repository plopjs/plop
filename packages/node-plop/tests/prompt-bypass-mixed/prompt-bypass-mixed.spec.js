...
describe('prompt-bypass-mixed', function () {
	...
	test('verify good bypass input', async function () {
		const [promptsAfterBypassOne, bypassOne] = await promptBypass(prompts, ['0'], plop);
		expect(bypassOne.list).toBe('a');
		expect(promptsAfterBypassOne.length).toBe(5);
		expect(promptsAfterBypassOne[0].type).toBe(undefined);

		const [promptsAfterBypassTwo, bypassTwo] = await promptBypass(prompts, ['b', 'something'], plop);
		expect(bypassTwo.list).toBe('B');
		expect(bypassTwo.input).toBe('something');
		expect(promptsAfterBypassTwo.length).toBe(4);

		const [promptsAfterBypassThree, bypassThree] = await promptBypass(prompts, ['b', 'something', 'something filtered'], plop);
		...
		await promptBypass(prompts, ['b', 'something', 'something filtered', 'unimportant'], plop);
	...
	// can't bypass conditional prompts
		await expect(() => promptBypass(prompts, ['a', 'something', 'something filtered', 'unimportant', 'something else'], {is: plop})).rejects.toThrow();
	});
});
