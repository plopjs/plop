...
describe('prompt-bypass-checkbox', function () {
	...
	test('verify good bypass input', async function () {
		...
		expect(JSON.stringify(allAnswersByValue.checkbox)).toBe('["one","two","three"]');
		...
		expect(JSON.stringify(allAnswersByIndex.checkbox)).toBe('["one","two","three"]');
		...
		expect(JSON.stringify(allAnswersByMixed.checkbox)).toBe('["one","two","three"]');
	...
	test('verify bad bypass input', async function () {
		await expect(() => promptBypass(prompts, ['one,four']).reject.toThrow( {is: plop}));
		await expect(() => promptBypass(prompts, ['four'],).reject.toThrow({is: plop}));
		await expect(() => promptBypass(prompts, ['3'],).reject.toThrow({is: plop}));
	});
});
