...
describe('basic-plopfile', function () {
	...
	test('Test the content of the rendered file _THIS_IS_A_TEST.txt', async () => {
		...
		expect(
			content.includes('propertyPathTest: basic-plopfile-test-propertyPath-value-index-1')
		).toBe(true);
	});
	...
	test('Test the content of the rendered file change-me.txt', async () => {
		...
		expect(content.includes('this is prepended! replaced => this-is-a-test: 21')).toBe(true);
	});
});
